...
## EP: Abstract Scenes doesn't have train/val/test splits
...
def main_encode_abstractscenes_images(cfg):
    clip = None #build_clip_encoder(cfg).cuda()
    resnet = build_resnet152_encoder(cfg).cuda()
    #resnet = build_resnet152_encoder(cfg)
    #resnet = build_resnet101_encoder(cfg).cuda()
    ...
    echo(f"Total {len(abstractscenes_loader)} / {len(abstractscenes_loader.dataset)} AbstractScenes batches.")
